...
describe("19. אין שינוי ב-dohefes_reports.tracking / RLS של dohefes_reports", () => {
  it("dohefes_reports מוזכרת רק כ-FK reference (report_id ... references dohefes_reports(id)) - לא ALTER/UPDATE/SELECT/INSERT עליה", () => {
    const linesTouchingReports = activeSql(migrationSql)
      .split("\n")
      .filter((line) => /\bdohefes_reports\b/.test(line));
    for (const line of linesTouchingReports) {
      expect(line).toMatch(/references dohefes_reports\(id\)/);
    }
    // עדיין חייבת להיות בדיוק שורת ה-FK האחת הצפויה - לא "אין מופעים בכלל" (זה עצמו היה חשוד -
    // הטבלה חייבת להיות מקושרת ל-dohefes_reports).
    expect(linesTouchingReports.length).toBe(1);
  });

  it("שום ALTER TABLE dohefes_reports / UPDATE dohefes_reports / SELECT ... FROM dohefes_reports בקובץ", () => {
    const active = activeSql(migrationSql).toLowerCase();
    expect(active).not.toMatch(/alter table dohefes_reports/);
    expect(active).not.toMatch(/update dohefes_reports/);
    expect(active).not.toMatch(/from dohefes_reports/);
  });

  it("אין שום SELECT/COPY מהעמודה tracking (dohefes_reports.tracking) - אין העתקת נתונים ישנים בשלב הזה", () => {
    expect(activeSql(migrationSql).toLowerCase()).not.toMatch(/dohefes_reports\.tracking/);
  });

...
export function buildWorkbook(inputs: ProjectInputs, result: ProjectResult): XLSX.WorkBook {
  ...
  const assumptionRows: (string | number)[][] = [
    ...
    ["שכירות חודשית ליחידה בשנה הראשונה (₪)", inputs.costs.relocationBaseMonthlyRentPerUnitNis],
    ["עלות הובלה ליחידה, הלוך ושוב (₪)", inputs.costs.relocationMovingCostPerUnitNis],
    ...inputs.costs.relocationYears.flatMap((year, index): (string | number)[][] => [
      [`שנה ${index + 1}, חודשים`, year.months],
      ...(index > 0 ? [[`שנה ${index + 1}, התייקרות מהשנה הקודמת (%)`, year.increasePct * 100]] : []),
    ]),
    [],

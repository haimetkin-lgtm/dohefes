...
describe("SITE_PATHS - basePath audit", () => {
  ...
  it("cashflow(reportId) כולל /dohefes פעם אחת ושומר את reportId ב-query string", () => {
  ...
  it("cashflow מקודד תווים מיוחדים ב-reportId (הגנה בסיסית, גם אם לא אמור לקרות בפועל - reportId הוא UUID תמיד)", () => {
    const url = SITE_PATHS.cashflow("abc def/../xyz");

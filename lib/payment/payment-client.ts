...
// נרשמת. הקורא (React) אחראי אם/איך להציג שגיאה למשתמש.
//
// **מקור: reconciliation מול gen2-cashflow-ui-implementation (Commit 5b)** - הובא כמות שהוא,
// ללא שינוי לוגי - כבר גנרי לחלוטין על ProductType (מיובא מ-payment-storage.ts, שכבר כולל
// trackingReports מ-Commit 2). לא נלקח app/cashflow/page.tsx או שום דבר ספציפי לתזרים.
...
/** מיוצא (לא רק פנימי) - נדרש גם ב-tracking-client.ts (Commit 5b), כדי לא לשכפל ולידציית
 *  מחרוזת-לא-ריקה בשני קבצים נפרדים. */
...
/** מיוצא - נדרש גם ב-tracking-client.ts (Commit 5b), אותה לוגיקת פענוח שגיאת HTTP בדיוק. */
export async function readHttpErrorBody(error: { context?: unknown }): Promise<{ status: number | null; body: unknown }> {
...
/** מיוצא - נדרש גם ב-tracking-client.ts (Commit 5b). */
export function errorReasonFromBody(body: unknown, fallback: string): string {
 ...
 *
 * מיוצא - אותו סיווג בדיוק משמש גם tracking-client.ts (Commit 5b, dohefes-get-tracking-data/
 * dohefes-save-tracking-data) - סיווג HTTP גנרי, לא ספציפי ל-create-payment-order.
 */

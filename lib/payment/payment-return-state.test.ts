...
describe("promoting -> redirecting/storage_error", () => {
  ...
  it("PROMOTE_SUCCEEDED -> redirecting עם reportId בלבד (לא חושף accessToken/paymentContextId הלאה)", () => {
    const next = reducePaymentReturnState(promotingState, { type: "PROMOTE_SUCCEEDED" });
    expect(next).toEqual({ kind: "redirecting", reportId: CTX.reportId });
  });
...
describe("אירועים לא-רלוונטיים למצב הנוכחי - מתעלמים, לא זורקים", () => {
  ...
  it("ACCESS_ACTIVE כשכבר redirecting - state לא משתנה (תשובה מאוחרת אחרי שכבר עברו הלאה)", () => {
    const state: PaymentReturnState = { kind: "redirecting", reportId: CTX.reportId };
    expect(reducePaymentReturnState(state, { type: "ACCESS_ACTIVE" })).toBe(state);
  });

  it("CONTEXT_FOUND כשכבר לא ב-resolving_context - state לא משתנה", () => {
    const state: PaymentReturnState = { kind: "unavailable" };

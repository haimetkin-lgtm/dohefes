...
function pendingInput(overrides: Partial<{ reportId: string; productType: ProductType; accessToken: string; checkoutUrl: string; idempotencyKey: string }> = {}) {
  return {
    reportId: REPORT_A,
...
describe("addPending + resolvePendingByContext", () => {
  ...
  it("trackingReports (מוצר שנוסף אחרי שהמאגר הזה נכתב לראשונה) עובר ולידציה תקין - isProductType, לא רשימה קשיחה ישנה", () => {
    const storage = new FakeStorage();
    addPending(storage, "po_tracking", pendingInput({ productType: "trackingReports" }), NOW);
    expect(resolvePendingByContext(storage, "po_tracking", NOW)?.productType).toBe("trackingReports");
  });
});
...
describe("JSON פגום נכשל סגור", () => {
  ...
  it("migration: רשומה מ-schemaVersion 1 הישן (בלי checkoutUrl/idempotencyKey) נכשלת סגור - לא מומרת בניחוש, לא זורקת", () => {
    const storage = new FakeStorage();
    // בדיוק הצורה שהמודול היה כותב לפני Commit 5b - schemaVersion 1, בלי checkoutUrl/idempotencyKey.
    storage.setRaw(
  ...
  it("productType לא-מוכר (למשל 'unitRanking' - לא productType אמיתי) נדחה כרשומה לא תקינה", () => {
    const storage = new FakeStorage();
    const raw = { schemaVersion: 2, entries: { po_bad: { reportId: REPORT_A, productType: "unitRanking", accessToken: "tok", checkoutUrl: CHECKOUT_URL, idempotencyKey: IDEMPOTENCY_KEY, createdAt: NOW.toISOString() } } };
    storage.setRaw("dohefes.pendingPurchases", JSON.stringify(raw));
    expect(resolvePendingByContext(storage, "po_bad", NOW)).toBeNull();
  });
});
...
describe("PendingPurchaseRecord - טיפוס מיוצא, כולל checkoutUrl/idempotencyKey", () => {

...
describe("addPending + resolvePendingByContext", () => {
  it("רשומה שנוספה נמצאת בדיוק לפי paymentContextId שלה", () => {
    const storage = new FakeStorage();
    const result = addPending(storage, "po_abc", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok-1" }, NOW);
    expect(result.ok).toBe(true);

    const resolved = resolvePendingByContext(storage, "po_abc", NOW);
    expect(resolved).toEqual({ reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok-1", createdAt: NOW.toISOString() });
  });
...
describe("שתי לשוניות - pending לא נדרס", () => {
  it("שתי הזמנות עם paymentContextId שונה נשמרות שתיהן, כל אחת נמצאת בנפרד", () => {
    const storage = new FakeStorage();
    addPending(storage, "po_tab1", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok-tab1" }, NOW);
    addPending(storage, "po_tab2", { reportId: REPORT_B, productType: "cashFlowAnalysis", accessToken: "tok-tab2" }, NOW);
...
describe("promoteToActive - סדר הפעולות: כתיבה ל-active לפני מחיקת pending", () => {
  it("במקרה הרגיל: ה-token עובר ל-productAccess, ורק אז ה-pending נמחק", () => {
    const storage = new FakeStorage();
    addPending(storage, "po_abc", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok-1" }, NOW);
...
describe("revokeActiveAccess - מוחקת רק report/product מתאים", () => {
  it("revoked למוצר אחד לא מוחק גישה פעילה למוצר אחר על אותו דוח, ולא לאותו מוצר על דוח אחר", () => {
    const storage = new FakeStorage();
    addPending(storage, "po_1", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok-cf-a" }, NOW);
    addPending(storage, "po_2", { reportId: REPORT_A, productType: "baseReport", accessToken: "tok-base-a" }, NOW);
    addPending(storage, "po_3", { reportId: REPORT_B, productType: "cashFlowAnalysis", accessToken: "tok-cf-b" }, NOW);
...
describe("TTL - pending שפג מנוקה, active אינו נמחק לפי אותו TTL", () => {
  it("cleanupPending מסירה רשומה שעברה את PENDING_TTL_MS, משאירה רשומה תקינה", () => {
    ...
    addPending(storage, "po_expired", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok-old" }, oldEnough);
    addPending(storage, "po_fresh", { reportId: REPORT_B, productType: "cashFlowAnalysis", accessToken: "tok-new" }, NOW);
  ...
  it("רשומה בדיוק בגבול ה-TTL (לא עברה אותו) עדיין נחשבת תקפה", () => {
    ...
    addPending(storage, "po_boundary", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok" }, justInside);
  ...
  it("pending באמצע polling (כמה קריאות resolvePendingByContext ברצף, בלי טעינת TTL) לא נמחק באמצע", () => {
    const storage = new FakeStorage();
    addPending(storage, "po_polling", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok" }, NOW);

    // מדמה polling - כמה קריאות רצופות, כולן עדיין הרבה לפני ה-TTL
    for (let i = 0; i < 10; i++) {
      const pollTime = new Date(NOW.getTime() + i * 2000);
      expect(resolvePendingByContext(storage, "po_polling", pollTime)?.accessToken).toBe("tok");
    }
  });
...
describe("localStorage quota error - לא זורק, מדווח ok:false", () => {
  it("addPending עם setItem שנכשל מחזירה ok:false ולא זורקת חריגה", () => {
    ...
    expect(() => addPending(storage, "po_abc", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok" }, NOW)).not.toThrow();
    const result = addPending(storage, "po_abc", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok" }, NOW);
    expect(result.ok).toBe(false);
  });
});
...
describe("אין מוטציה של הקלט", () => {
  it("addPending לא משנה את אובייקט ה-input שהועבר לה", () => {
    const storage = new FakeStorage();
    const input = { reportId: REPORT_A, productType: "cashFlowAnalysis" as const, accessToken: "tok-1" };
...
describe("אין access token בפלט שגיאה/חריגה", () => {
  it("כשל כתיבה (quota) - הודעת השגיאה הפנימית שנתפסת אינה כוללת את ה-accessToken בשום מקום שנחשף", () => {
    ...
    const result = addPending(storage, "po_abc", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: secretToken }, NOW);
  ...
  it("promoteToActive עם כשל - ה-result המוחזר לא כולל את הטוקן", () => {
    const storage = new FakeStorage();
    const secretToken = "super-secret-access-token-xyz";
    addPending(storage, "po_abc", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: secretToken }, NOW);
    storage.failSetItemKeys.add("dohefes.productAccess");
    const result = promoteToActive(storage, "po_abc", NOW);
    expect(JSON.stringify(result)).not.toContain(secretToken);
  });
...
describe("fallback לפי reportId+productType (resolvePendingByReportAndProduct)", () => {
  it("התאמה יחידה נמצאת בהצלחה, כולל paymentContextId שלה", () => {
    const storage = new FakeStorage();
    addPending(storage, "po_only", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok" }, NOW);

    const result = resolvePendingByReportAndProduct(storage, REPORT_A, "cashFlowAnalysis", NOW);

    expect(result).toEqual({ ok: true, paymentContextId: "po_only", record: { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok", createdAt: NOW.toISOString() } });
  });
  ...
  it("מוצר שונה על אותו דוח לא נחשב התאמה", () => {
    const storage = new FakeStorage();
    addPending(storage, "po_base", { reportId: REPORT_A, productType: "baseReport", accessToken: "tok" }, NOW);
    const result = resolvePendingByReportAndProduct(storage, REPORT_A, "cashFlowAnalysis", NOW);
    expect(result).toEqual({ ok: false, reason: "not_found" });
  });
  ...
  it("שתי התאמות סותרות (אותו report+product, שני paymentContextId שונים) - נכשל סגור, לא בוחר אחת", () => {
    const storage = new FakeStorage();
    addPending(storage, "po_first", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok-1" }, NOW);
    addPending(storage, "po_second", { reportId: REPORT_A, productType: "cashFlowAnalysis", accessToken: "tok-2" }, NOW);

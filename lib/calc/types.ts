...
/**
 * שנת תשלום דמי שכירות אחת בתקופת הפינוי (ר' CostInputs.relocationYears). ההתייקרות היא
 * מצטברת/מתגלגלת - ביחס לשכ"ד החודשי של השנה הקודמת ברשימה, לא ביחס לשכ"ד הבסיס של השנה
 * הראשונה (ר' engine.ts::relocationMonthlyRentByYear לנוסחה המדויקת).
 */
export interface RelocationYear {
  /** חודשים בשנה הזו - בדרך כלל 12, ניתן לשינוי לשנה חלקית (למשל שנה אחרונה) */
  months: number;
  /**
   * שיעור ההתייקרות ביחס לשכ"ד החודשי של השנה הקודמת ברשימה, כשבר (0.05 = 5%). לא רלוונטי
   * בשנה הראשונה (אין שנה קודמת) - השדה עדיין קיים שם לאחידות המבנה, פשוט אינו נקרא בחישוב.
   */
  increasePct: number;
}
...
export interface CostInputs {
  ...
  // שמתפנים), פרוסים על שנים עם התייקרות אופציונלית לכל שנה, בתוספת עלות הובלה חד-פעמית.
  // מספר היחידות הוא נתון פיזי שהסוכן החכם יכול להעריך מהתיאור/תוכניות, שאר הערכים כספיים
  // שהמשתמש תמיד מזין בעצמו.
  /** מספר יחידות קיימות הזכאיות לדמי שכירות והובלה. 0 אם אין דיירים קיימים שמתפנים */
  relocationUnitsCount: number;
  /** שכ"ד חודשי ליחידה בשנה הראשונה, ₪ - הבסיס שממנו מתייקרות השנים הבאות (ר' RelocationYear) */
  relocationBaseMonthlyRentPerUnitNis: number;
  /**
   * שנות תשלום דמי השכירות, לפי הסדר - שנה ראשונה ואילך. מערך ריק = אין תשלום שכירות בכלל.
   * `increasePct` בשנה הראשונה (אינדקס 0) אינו נקרא בחישוב (אין שנה קודמת להתייקר ממנה).
   */
  relocationYears: RelocationYear[];
  /** עלות הובלה ליחידה, חד-פעמית - כוללת הובלה לדירה השכורה ובחזרה לדירה החדשה בתום הבנייה, ₪ */
  relocationMovingCostPerUnitNis: number;
...
export interface CostBreakdown {
  ...
  /** עלות הובלה חד-פעמית לדיירים הקיימים (הלוך ושוב), כבר כלול ב-indirectNis, מוצג כאן גם בנפרד לתצוגה */
  relocationMovingCostNis: number;
